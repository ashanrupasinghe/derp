	...
	multiField.prototype = {
      ...
      if(this.config.locale !== 'default'){
			...
			this.$elem.on('click',this.config.btnAdd,function(e){
				e.preventDefault();				
				$('select').select2('destroy');//1.destroi select2
				$this.cloneSection();
				$('select').select2();//2.add select 2
				/*above 1,2, line added by ashan,
				 *when cloning field set,normaly select 2 work on first original
				 *befor clone we can remove select2 and after clone add again select 2 
				 * */ 
			});
		...
		cloneSection : function() {
			...
			$('select.sup-select', newChild).each(function () {
				$(this).empty();
			});
			...
			// Append new section
			this.$elem.append(newChild);//prepend
			//$('.dinamic-group').append(newChild);//prepend , change by ashan
